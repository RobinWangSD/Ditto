...
class LocalPoolPointnetPPFusion(nn.Module):
    ...
    def forward(self, p, p2):
        batch_size, T, D = p.size() # T: num_points, D: dim(3) 
        ...
        if "grid" in " ".join(self.plane_type):
            coord["grid"] = normalize_3d_coordinate(p.clone(), padding=self.padding) # 32xTx3
            index["grid"] = coordinate2index(                                        # 32x1xT
        ...
        for f in self.plane_type:
            ...
            elif k2 == "grid":     # always use -geo for grid features
                if k1 == "geo":
